...
public class ReportParametersComponent extends AbstractLazyLoaderComponent {
	...
	// public ReportParametersComponent(String reportSource, Integer cacheId,
	// boolean lazyLoad) throws AperteReportsException {
	// this.reportSource = reportSource;
	// this.cacheId = cacheId;
	// if (!lazyLoad) {
	// reportMaster = new ReportMaster(reportSource, cacheId.toString(), new
	...
	// boolean includeReportFormat, boolean lazyLoad) throws
	// AperteReportsException {
	// this.reportSource = reportSource;
	// this.cacheId = cacheId;
	// this.reportParameters = reportConfig != null ?
	// reportConfig.getParameters() : null;
	...
	// boolean includeReportFormat, boolean lazyLoad, boolean readonly) throws
	// AperteReportsException {
	// this.reportSource = reportSource;
	// this.cacheId = cacheId;
	 ...
	 * its properties. Returns <code>null</code> on
	 * {@link BuildingFailedException} which may occur on special script
	 * invocation.
	 *
	 * @param param
	 *            Report parameter
	 ...
	 *            Field container
	 * @param field
	 *            Field the attributes are applied to
	 */
	protected void applyAttributes(FieldProperties fieldProperties, FieldContainer container, Field field) {
		...
		if (StringUtils.isNotEmpty(fieldProperties.getRegexp())) {
			field.addValidator(new RegexpValidator(fieldProperties.getRegexp(), VaadinUtil.getValue(fieldProperties.getRegexpError(),
					fieldProperties.getCaption(), fieldProperties.getRegexp())));
		}
	...
	protected void attachSpecialValidators(FieldProperties fieldProperties, FieldContainer container) throws BuildingFailedException {
		if (fieldProperties.getSei() != null && StringUtils.isNotEmpty(fieldProperties.getSpecialValidationCode())) {
			try {
				...
				Validator validator = (Validator) fieldProperties.getSei().invokeFunction("specialValidator", container,
						fieldProperties.getSpecialValidationError());
				container.addValidator(validator);
			} catch (ScriptException e) {
				NotificationUtil.showExceptionNotification(getWindow(), "invoker.form.special_validation_code.script_exception", e);
				logger.error(e.getMessage(), e);
				throw new BuildingFailedException(e);
			} catch (NoSuchMethodException e) {
				NotificationUtil.showExceptionNotification(getWindow(), "invoker.form.special_validation_code.no_such_method_exception", e);
				logger.error(e.getMessage(), e);
				throw new BuildingFailedException(e);
			}
		}
	 ...
	 * @throws BuildingFailedException
	 *             on special control building error
	 */
	protected Field buildFieldBaseForInputType(FieldProperties fieldProperties, List<DictionaryItem> items, FieldContainer container)
			throws BuildingFailedException {
		Field field = null;
		...
		case DATE:
			...
			dateField.setDateFormat(VaadinUtil.getValue(UiIds.COMMON_TIMEDATE_FORMAT));
		...
		case SPECIAL_CONTROL:
			...
			if (fieldProperties.getSei() != null && StringUtils.isNotEmpty(fieldProperties.getSpecialControlCode())) {
				try {
					...
					fieldProperties.getSei().invokeFunction("specialControl", container, items, fieldProperties.getWidth(), fieldProperties.getMaxchars(),
							fieldProperties.isRequired(), fieldProperties.getRequiredError(), fieldProperties.getRegexp(), fieldProperties.getRegexpError(),
							fieldProperties.getCaption());
				} catch (ScriptException e) {
					NotificationUtil.showExceptionNotification(getWindow(), "invoker.form.special_control_code.script_exception", e);
					logger.error(e.getMessage(), e);
					throw new BuildingFailedException(e);
				} catch (NoSuchMethodException e) {
					NotificationUtil.showExceptionNotification(getWindow(), "invoker.form.special_control_code.no_such_method_exception", e);
					logger.error(e.getMessage(), e);
					throw new BuildingFailedException(e);
				}
			}
	...
	public List<DictionaryItem> executeSpecialDataQueryCode(FieldProperties fieldProperties, List<DictionaryItem> items) throws BuildingFailedException {
		if (fieldProperties.getSei() != null && StringUtils.isNotEmpty(fieldProperties.getSpecialDataQueryCode())) {
			try {
				...
				logger.error(e.getMessage(), e);
				NotificationUtil.showExceptionNotification(getWindow(), "invoker.form.special_data_query_code.script_exception", e);
				throw new BuildingFailedException(e);
			} catch (NoSuchMethodException e) {
				logger.error(e.getMessage(), e);
	...
	/**
	 * Retrieves the datepatternVlaue from a report parameter property. If no
	 * value is set, default will be returned.
	 * 
	 * @param props
	 *            Input parameter properties
	 * @param key
	 ...
	 */
	private String getValueFromMap(Map<ARConstants.Keys, ReportProperty> props, ARConstants.Keys key, String defaultValue) {
		if (props.containsKey(key)) {
			String value = props.get(key).getValue();
	...
	private void initFieldsFromConfig() {
		...
		for (ReportConfigParameter param : params) {
			...
			// xxx maybe it could be easier to have linked map as parameters
			// instead of list
			...
			for (FieldContainer field : fields) {
				if (param.getName().equals(field.getName())) {
					try {
						...
						Class<?> fieldType = fieldComponent == null ? null : fieldComponent.getPropertyDataSource() == null ? null : fieldComponent
								.getPropertyDataSource().getType();
						if (DATE.equals(field.getComponentType())) {
							fieldType = Date.class;
						} else if (MULTISELECT.equals(field.getComponentType()) || CHECKBOXES.equals(field.getComponentType())
	...
	private void initView() {
		// at the moment there is a chance that this method will be invoked
		// twice per instance
	...
	private FieldProperties parseFieldProperties(ReportParameter param, Map<ARConstants.Keys, ReportProperty> props) {
		...
		// INPUT_TYPE
		String inputTypeString = getValueFromMap(props, ARConstants.Keys.INPUT_TYPE, ARConstants.InputTypes.values(), ARConstants.InputTypes.CHECKBOX.name());
		...
		// REQUIRED
		String requiredString = getValueFromMap(props, ARConstants.Keys.REQUIRED, ARConstants.BooleanValues.values(), "false");
		fieldProperties.setRequired(Boolean.valueOf(requiredString));
		fieldProperties.setRequiredError(getValueFromMap(props, ARConstants.Keys.REQUIRED_ERROR, null, "form.errors.required"));
		...
		// SELECT_ALL
		String selectAllString = getValueFromMap(props, ARConstants.Keys.SELECT_ALL, ARConstants.BooleanValues.values(), "false");
		fieldProperties.setSelectAll(Boolean.valueOf(selectAllString));

		// MULTIPLE_CHOICE
		String multipleChoiceString = getValueFromMap(props, ARConstants.Keys.MULTIPLE_CHOICE, ARConstants.BooleanValues.values(), "false");
